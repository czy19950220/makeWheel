<template>
  <div class="content-box">
    ...
    <czy-anchor>
      <czy-anchor-link-simplebar href="input-simple" title="基础用法" />
      <czy-anchor-link-simplebar href="input-pattern" title="模式" />
      <czy-anchor-link-simplebar href="input-disabled" title="禁用" />
      <czy-anchor-link-simplebar href="api" title="API" />
    </czy-anchor>

    <!-- 基础用法DEMO -->
    <czy-el-demo-block :height="312" :title="'基础用法'" id="input-simple">
      <template v-slot:demo>
        <czy-input v-model="simple.input" />
    ...
    <!-- 禁用状态DEMO -->
    <czy-el-demo-block :height="375" :title="'禁用状态'" id="input-disabled">
      <template v-slot:demo>
        <div style="padding:0 0 15px">
          <div style="display:inline-block;width:100px;font-size:14px">line模式</div>
          <czy-input :pattern="'line'" disabled v-model="disabled.line_input" />
          <span style="margin-left:20px">{{disabled.line_input}}</span>
        </div>
        <div style="padding:0 0 15px">
          <div style="display:inline-block;width:100px;font-size:14px">frame模式</div>
          <czy-input :pattern="'frame'" disabled v-model="disabled.frame_input" />
    ...
    return {
      ...
      pattern: {
        code: {
          html: `
          <template>
            <czy-input v-model="line_input" :pattern="'line'"/>
            <span style="margin-left:20px">{{line_input}}</span>
            <czy-input v-model="frame_input" :pattern="'frame'" />
      ...
      disabled: {
        code: {
          html: `
          <template>
            <czy-input v-model="line_input" :pattern="'line'" disabled/>
            <span style="margin-left:20px">{{line_input}}</span>
            <czy-input v-model="frame_input" :pattern="'frame'" disabled/>
            <span style="margin-left:20px">{{frame_input}}</span>
          </template>
          `,
          javascript: `
