<template>
  <div
          :class="[{'czy-anchor':!customize,'czy-anchor-left':position == 'left'},[anchorClass]]"
  >
    <div class="czy-anchor-ink">
      <span
              :class="{'visible':linkBallShow}"
              :style="{top:linkBallTop + 'px'}"
              class="czy-anchor-ink-ball"
      ></span>
    </div>
    <div class="czy-anchor-box">
      <slot></slot>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      activeTitle() {
        for (let i = 0; i < this.$slots.default.length; i++) {
          if (this.index == i) {
            this.$slots.default[i].elm.classList.add('czy-anchor-link-active')
          } else {
            this.$slots.default[i].elm.classList.remove('czy-anchor-link-active')
          }
        }
      }
    }
