<template>
  <div class="czy-menu-simple">
    <ul class="czy-meun-simple-nav">
      <div class="czy-menu-simple-nav-active-bar" ref="menuActiveBar"></div>
      <li
        :key="item.text"
...
export default {
  ...
  mounted() {
    ...
    for (
      let i = 0;
      i < document.getElementsByClassName('czy-meun-simple-li').length;
      i++
    ) {
      this.liWidth.push(
        document.getElementsByClassName('czy-meun-simple-li')[i].clientWidth
      )
    }
...
.czy-menu-simple {
  ...
  .czy-meun-simple-nav {
    ...
    .czy-menu-simple-nav-active-bar {
      position: absolute;
      bottom: 0;
      left: 0;
      height: 2px;
