  ...
  <div class="quarter-panel">
    ...
    <div class="quarter-panel-box">
      <div class="quarter-panel-box_select">
        <p>Start</p>
        <i @click="handelRange('start','last')" class="czyi czy-doubleleft"></i>
    ...
    <div class="quarter-panel-box">
      <div class="quarter-panel-box_select">
        ...
        <i @click="handelRange('end','next')" class="czyi czy-doubleright"></i>
      </div>
      <div class="quarter-panel-box_list clearfloat">
        <div
...
export default {
  ...
  watch: {
    value: function(val) {
      ...
      this.startDateSelect = this.startYear = czyUiDate.getYear(this.startDate)
      this.endDateSelect = this.endYear = czyUiDate.getYear(this.endDate)

      this.start = czyUiDate.getMonth(this.startDate)
      this.end = czyUiDate.getMonth(this.endDate)
    }
  },
  data() {
    return {
    ...
    handelSelect(dateType, item) {
      if (dateType == 'start') {
        if (!this.disabled('start', item)) {
          this.$emit('change', [
            this.startDateSelect + '-' + czyUiDate.appendZero(item) + '-01',
            this.endDate
          ])
        }
      } else {
        if (!this.disabled('end', item)) {
          this.$emit('change', [
            ...
            this.endDateSelect +
              '-' +
              czyUiDate.appendZero(item) +
              '-' +
              czyUiDate.getLastDayOfMonth(this.endDateSelect, item)
          ])
        }
      }
    },
