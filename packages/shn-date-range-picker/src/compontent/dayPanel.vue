  ...
  <div class="day-panel">
    ...
    <div class="day-panel-box" v-if="type == 'day'">
      <div class="day-panel-box_select">
        ...
        <i @click="handelRangeMonth('end','next')" class="czyi czy-right"></i>
        <i @click="handelRangeYear('end','next')" class="czyi czy-doubleright"></i>
      </div>
      <div class="day-panel-box_list clearfloat">
        <table cellpadding="0" cellspacing="0">
          <tbody>
...
export default {
  ...
  watch: {
    value: function(val) {
      ...
      this.startSelectYear = this.startYear = czyUiDate.getYear(this.startDate)
      this.endSelectYear = this.endYear = czyUiDate.getYear(this.endDate)
      this.startSelectMonth = this.startMonth = czyUiDate.getMonth(this.startDate)
      this.endSelectMonth = this.endMonth = czyUiDate.getMonth(this.endDate)

      this.start = czyUiDate.getDay(this.startDate)
      this.end = czyUiDate.getDay(this.endDate)
    }
  },
  data() {
    return {
  ...
  computed: {
    getFirstTr() {
      return function(type) {
        let oneDayWeek = czyUiDate.getWeek(
        ...
        let lastDayOfMonth = czyUiDate.getLastDayOfMonth(
          type == 'start' ? this.startSelectYear : this.endSelectYear,
  ...
  methods: {
    ...
    handelSelect(dateType, item) {
      ...
      if (this.type == 'day') {
        ...
        this.$emit('change', czyUiDate.getSomeWeek(item.date))
      }
    },
    inSelect(td, type) {
      if (this.type == 'day') {
    ...
    inRange(td) {
      ...
      let e = new Date(this.endDate).getTime()
      for (let i = 0; i < td.length; i++) {
        td[i].date = `${td[i].year}-${czyUiDate.appendZero(
          td[i].month
        )}-${czyUiDate.appendZero(td[i].value)}`
